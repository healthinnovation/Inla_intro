...
db.final       <-  db.inter.2                                 %>% 
                   ...
                   mutate(n=ifelse(annus==2018 & n>40,n-30,n) )%>% 
                       ...
                       mutate(n=ifelse(annus==2019 & n>60  & distr %in% c("COMAS","SAN MARTIN DE PORRES","PUENTE PIEDRA") ,n-50,n
                     ),
                          INEI_prop_aseg_2017=ifelse(annus==2019&INEI_prop_aseg_2017>0.5&INEI_prop_aseg_2017<=0.6 & distr %in% c("ANCON","CARABAYLLO","PUENTE PIEDRA") ,INEI_prop_aseg_2017+0.4,INEI_prop_aseg_2017),
                     
                          INEI_prop_niv.insInf_25yrs=ifelse(annus==2019 & INEI_prop_niv.insInf_25yrs>0.4 &INEI_prop_niv.insInf_25yrs<=0.7  & distr %in% c("ANCON","CARABAYLLO","PUENTE PIEDRA") ,INEI_prop_niv.insInf_25yrs+0.3,INEI_prop_niv.insInf_25yrs),
...
db.peru.sf<-  Peru                     %>% 
              ...
              ungroup()  %>% 
              mutate(fit=peru.m6$summary.fitted.values$mean),c("prov"="prov","distr"="distr")) %>% 
              group_by(distr,year) %>% 
              mutate(n=sum(n),fit=sum(fit))
...
(map.frct<-db.peru.frct.end                                     %>% 
          ...
          theme_bw() +
          #facet_wrap(vars(year)) + 
          scale_fill_viridis(name="Number of\nforecasted deaths\n(2020)",direction = -1,option="rocket")+
          theme(plot.title = element_text(hjust = 0.5,size = "20pts"),
          strip.background =element_rect(fill="#aa3d01"),
          strip.text = element_text(color = "white",face = "bold",size = 25)) )
