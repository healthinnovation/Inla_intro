...
```{r,message=F,warning=F,echo=F,eval=F}
db.extra<-readRDS("db_excess_proc_dis_1819_m.rds")
...
db.frcst<-         db.inter.2                                   %>% 
                   ...
                   mutate(n=ifelse(distr=="LIMA",NA,n))          %>% 
                   rename(year           = annus,
                          month          = mes,
                          pp.insured     = INEI_prop_aseg_2017    ,
                          pp.pover       = prop_pobreza_2018      ,
                          pp.no.elec     = INEI_Prop_NoAlumbradoElectri_2017,
                          pp.no.water    = INEI_prop_NoAbsAguVvn_int_2017,
                          pp.edu.under25 =  INEI_prop_niv.insInf_25yrs
                          )
# %>% 
...
lima.m2 <- inla(n ~ 1 + year + temperature+ pp.insured + pp.pover + pp.no.elec + pp.no.water,
                  verbose           = F,
                  data              =  db,
                  family            = "nbinomial",
                  control.compute   = list(dic=TRUE, waic=TRUE,cpo=TRUE),
...
To include these type of priors as spatial effects, first we  need to collect the geographic information of Peru expressed in a `shapefile`, at the district level. For which, we use the shapefile of Lima 
...
f.m3<-lima.m3$summary.fixed %>% mutate(Model="linear")      %>% rownames_to_column("Variable")
f.m4<-lima.m4$summary.fixed %>% mutate(Model="linearRW(1)") %>% rownames_to_column("Variable")
f.m5<-lima.m5$summary.fixed %>% mutate(Model="AR(1)RW(1)")  %>% rownames_to_column("Variable")
...
```{r,message=F,warning=F,eval=F}
data.cpo                                                                    %>% 
pivot_longer(cols=colnames(datos),names_to = "model",values_to = "CPO")     %>% 
gt()                                                                        %>%
tab_header(
    title = md("LOO-CV")
    ) 
