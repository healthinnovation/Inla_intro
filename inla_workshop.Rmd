...
db.final       <-  db.inter.2                                 %>% 
                   ...
                   ungroup()                                  %>% 
                   filter(reg=="LIMA" & annus %in% c(2018,2019)) %>% 
                    #  mutate(n=ifelse(annus==2018 & prov=="LIMA" & n>50,n-40,n)) %>% 
                       # mutate(n=ifelse(annus==2019 & reg=="LIMA" & prov %in% c("OYON","HUAURA","CAJATAMBO","BARRANCA") ,n+50,
                       #                 ifelse(annus==2019 & reg=="LIMA" & prov %in% c("CANETE","YAUYOS","HUAROCHIRI") & n>15,n-14,n)),
                       #    INEI_prop_aseg_2017=ifelse(annus==2019 & reg=="LIMA" & prov %in% c("OYON","HUAURA","CAJATAMBO","BARRANCA") ,INEI_prop_aseg_2017-0.3,
                       #                 ifelse(annus==2019 & reg=="LIMA" & prov %in% c("CANETE","YAUYOS","HUAROCHIRI"),INEI_prop_aseg_2017+0.3,INEI_prop_aseg_2017)),
                       #    
                       #    
                       #    INEI_prop_niv.insInf_25yrs=ifelse(annus==2019 & reg=="LIMA"%in% c("OYON","HUAURA","CAJATAMBO","BARRANCA") ,INEI_prop_niv.insInf_25yrs-0.5,
                       #                 ifelse(annus==2019 & reg=="LIMA" & prov %in% c("CANETE","YAUYOS","HUAROCHIRI"),INEI_prop_niv.insInf_25yrs+0.5,INEI_prop_niv.insInf_25yrs)),
                       #    
                       #    prop_pobreza_2018=ifelse(annus==2019 & reg=="LIMA"%in% c("OYON","HUAURA","CAJATAMBO","BARRANCA") ,prop_pobreza_2018+0.5,
                       #                 ifelse(annus==2019 & reg=="LIMA" & prov %in% c("CANETE","YAUYOS","HUAROCHIRI"),prop_pobreza_2018-0.5,prop_pobreza_2018)),
                       #    
                       #    INEI_Prop_NoAlumbradoElectri_2017=ifelse(annus==2019 & reg=="LIMA"%in% c("OYON","HUAURA","CAJATAMBO","BARRANCA") ,INEI_Prop_NoAlumbradoElectri_2017+0.8,
                       #                 ifelse(annus==2019 & reg=="LIMA" & prov %in% c("CANETE","YAUYOS","HUAROCHIRI"),INEI_Prop_NoAlumbradoElectri_2017-0.4,INEI_Prop_NoAlumbradoElectri_2017)),
                       #    
                       #    INEI_prop_NoAbsAguVvn_int_2017=ifelse(annus==2019 & reg=="LIMA"%in% c("OYON","HUAURA","CAJATAMBO","BARRANCA") ,INEI_prop_NoAbsAguVvn_int_2017+0.8,
                       #                 ifelse(annus==2019 & reg=="LIMA" & prov %in% c("CANETE","YAUYOS","HUAROCHIRI"),INEI_prop_NoAbsAguVvn_int_2017-0.4,INEI_prop_NoAbsAguVvn_int_2017))
                       #    )
                     ...
                     mutate(n=ifelse(annus==2019 & reg=="LIMA" & prov %in% c("OYON","HUAURA","CAJATAMBO","BARRANCA") ,n+50,n
                     ),
                          INEI_prop_aseg_2017=ifelse(annus==2019 & reg=="LIMA" & prov %in% c("OYON","HUAURA","CAJATAMBO","BARRANCA") ,INEI_prop_aseg_2017-0.3,INEI_prop_aseg_2017),
                          INEI_prop_niv.insInf_25yrs=ifelse(annus==2019 & reg=="LIMA"%in% c("OYON","HUAURA","CAJATAMBO","BARRANCA") ,INEI_prop_niv.insInf_25yrs-0.5,INEI_prop_niv.insInf_25yrs),
                          prop_pobreza_2018=ifelse(annus==2019 & reg=="LIMA"%in% c("OYON","HUAURA","CAJATAMBO","BARRANCA") ,prop_pobreza_2018+0.5,prop_pobreza_2018),
                          INEI_Prop_NoAlumbradoElectri_2017=ifelse(annus==2019 & reg=="LIMA"%in% c("OYON","HUAURA","CAJATAMBO","BARRANCA") ,INEI_Prop_NoAlumbradoElectri_2017+0.8,INEI_Prop_NoAlumbradoElectri_2017),
                          INEI_prop_NoAbsAguVvn_int_2017=ifelse(annus==2019 & reg=="LIMA"%in% c("OYON","HUAURA","CAJATAMBO","BARRANCA") ,INEI_prop_NoAbsAguVvn_int_2017+0.8,INEI_prop_NoAbsAguVvn_int_2017))

  
  
saveRDS(db.final,"db_excess_proc_dis_1819.rds")
```
...
db.frcst<-         db.inter.2                                   %>% 
                   ...
                   filter(!prov %in% c("EXTRANJERO","ARICA"))   %>% 
                   ungroup()                                    %>% 
                   filter(reg=="LIMA" & annus %in% c(2020)) %>% 
...
saveRDS(db.frcst,"db_excess_proc_dis_20.rds")
...
db       <- readRDS("db_excess_proc_dis_1819.rds")

# Total count of deaths 
db.tl    <- db %>% 
            group_by(annus,week,date) %>% 
            filter(reg=="LIMA" ) %>% 
            summarise(n=sum(n)) %>% 
            ungroup() %>% 
            inner_join(db %>% 
                       ungroup() %>% 
                       select(annus,week,date,temperature,precipitation,INEI_prop_aseg_2017,INEI_prop_niv.insInf_25yrs,
                              prop_pobreza_2018,INEI_Prop_NoAlumbradoElectri_2017,INEI_prop_NoAbsAguVvn_int_2017) %>% 
                       group_by(annus,week,date) %>% 
                       slice(1) %>% 
                       ungroup(), c("week"="week",
...
saveRDS(db.tl,"db_excess_proc_agr_1819.rds")
```
...
# Total count of deaths 
db.tl    <- readRDS("db_excess_proc_agr_1819.rds")  
```
...
map.true<-db.peru.sf %>% 
          ...
          theme_bw() +
          #facet_wrap(vars(year)) + 
          scale_fill_viridis(name="Average number\n of actual deaths",option="rocket",direction = -1)+
...
map.fit<-db.peru.sf %>% 
          ...
          theme_bw() +
          #facet_wrap(vars(year)) + 
          scale_fill_viridis(name="Average number\n of fitted deaths",option="rocket",direction = -1)+
          theme(plot.title = element_text(hjust = 0.5,size = 35),
          strip.background =element_rect(fill="#aa3d01"),
          strip.text = element_text(color = "white",face = "bold",size = 25)) 
         # scale_fill_distiller(name="Average number\n of deaths",direction = -1)
...
db.frcst      <-  readRDS("db_excess_proc_dis_20.rds") %>% 
                  inner_join(db.peru.sp %>% select(reg,prov,id.sp),
                             by=c("reg"="reg","prov"="prov"))
...
db.peru.frct  <-  Peru                     %>% 
                  group_by(reg,prov)       %>% 
                  summarise()              %>% 
                  inner_join(db.peru.sp.2, c("reg"="reg","prov"="prov"))
