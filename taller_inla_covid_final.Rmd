...
Asimismo procemos a asignar toda la informacion de los valores ajustados de los modelos a un objeto lista. Posteriormente empleamos `melt` para crear una estructura de base de datos adecuada para graficar .Asi como recolectamos informacion de los intervalos de credibilidad de los valores ajustados : `lima.m.m1$summary.fitted.values$0.025quant`, `lima.m.m1$summary.fitted.values$0.975quant`,
```{r}
...
Para el calculo de metricas en la búsqueda de evaluar los modelo propuestos emplearemos el paquete `yardstick` . En particular empleamos la funcion `metric_set` para seleccionar la metricas que deseamos calcular (`mae`,`rmse`,`msd`). Agrupamos los datos por cada modelo para realizar el calculo a ese nivel y finalmente empleamos la funcion `perform.metrics`, para lo cual detallamos los parametros `truth` y `estimate` que recogen los datos verdaderos y ajustados.
...
Como se puede ver no necesariamente el modelo con efectos espaciales obtiene un mejor performance respecto de los otros modelos.  Y es que los modelos con efectos aleatorios espaciales si bien gozan de buen ajuste bajo el enfoque de prediccion optado en este tutorial, la prediccion no es necesariamente mejor que modelos más sencillos .
